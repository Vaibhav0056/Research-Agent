...
# ── Fail fast on missing secrets, with a clean message (no traceback) ─────────
try:
    SETTINGS.validate()
...
# Each stage is a pure(ish) string->string function so Streamlit can memoize it.
# Identical topics reuse results instead of re-billing the LLM/search APIs.
...
@st.cache_data(ttl=3600, show_spinner=False)
def run_reader(topic: str, search_results: str) -> str:
    ...
    result = agent.invoke({
        "messages": [("user",
            ...
            f"Search Results:\n{search_results[: SETTINGS.handoff_max_chars]}"
        )]
    })
    return result["messages"][-1].content
...
def run_writer(topic: str, research: str) -> str:
    ...
    return get_writer_chain().invoke({"topic": topic, "research": research})
...
def run_critic(report: str) -> str:
    ...
    return get_critic_chain().invoke({"report": report})
...
# ── Session state init ────────────────────────────────────────────────────────
_defaults = {"results": {}, "running": False, "done": False, "error": None, "run_id": None}
...
if run_btn:
    ...
    elif len(cleaned) > SETTINGS.max_topic_chars:
        st.warning(
            f"Topic is too long ({len(cleaned)} chars). "
            f"Please keep it under {SETTINGS.max_topic_chars} characters."
        )
    else:
...
if st.session_state.running and not st.session_state.done:
    ...
    run_id = st.session_state.get("run_id", new_run_id())
    logger.info("run=%s | pipeline start (topic=%r)", run_id, topic_val)
    ...
    try:
        ...
        with st.spinner("📄  Reader Agent is scraping top resources…"):
            results["reader"] = run_reader(topic_val, results["search"])
            st.session_state.results = dict(results)
        ...
        with st.spinner("✍️  Writer is drafting the report…"):
            ...
            results["writer"] = run_writer(topic_val, research_combined)
            st.session_state.results = dict(results)

        # ── Step 4: Critic ──
        with st.spinner("🧐  Critic is reviewing the report…"):
            results["critic"] = run_critic(results["writer"])
            st.session_state.results = dict(results)

        logger.info("run=%s | pipeline complete", run_id)

    except Exception as exc:  # noqa: BLE001 - show a clean message, never a traceback
        logger.exception("run=%s | pipeline failed", run_id)
        st.session_state.error = (
            "The research pipeline hit an error and couldn't finish. "
...
if r:
    ...
    # NOTE: search/reader content comes from the live web and is UNTRUSTED.
    # It is HTML-escaped before being placed inside our styled container to
    # prevent HTML/script injection (XSS) into the page.
    if "search" in r:
